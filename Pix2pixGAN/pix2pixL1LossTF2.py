...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        print(tf.shape(raw_input)[0], tf.shape(raw_input)[1], tf.shape(raw_input)[2])
    ...
    def transform(image):
        ...
        offset = tf.cast(tf.floor(tf.random.uniform([2], 0, a.scale_size - CROP_SIZE + 1, seed=seed)), dtype=tf.int32)
...
def create_model(inputs, targets):
    ...
    with tf.name_scope("discriminator_loss"):
        # minimizing -tf.math.log will try to get inputs to 1
        # predict_real => 1
        # predict_fake => 0
    ...
    with tf.name_scope("generator_loss"):
        ...
        # gen_loss_per = compute_perceptual_loss(targets, outputs)
        # gen_loss = gen_loss_GAN * a.gan_weight + gen_loss_L1 * a.l1_weight + gen_loss_per
    ...
    ema = tf.compat.v1.train.ExponentialMovingAverage(decay=0.99)
