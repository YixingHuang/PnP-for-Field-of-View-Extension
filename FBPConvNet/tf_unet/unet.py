...
#    channels = 1
    ...
    pools = OrderedDict()
    deconv = OrderedDict()
    # Ling's modify: 'avg_unpool' replaces 'deconv';
    up_avg_unpool = OrderedDict()
    ...
    for layer in range(0, layers):
        ...
        #conv2 = conv2d(tmp_h_conv, w2, keep_prob)#FIX
        # Ling's modify: add BN after ReLu
        tmp_h_conv_BN = batch_norm_wrapper(tmp_h_conv, is_training)
        conv2 = conv2d(tmp_h_conv_BN, w2, keep_prob)
        #dw_h_convs[layer] = tf.nn.relu(conv2 + b2) #FIX
        dw_h_convs[layer] = batch_norm_wrapper(tf.nn.relu(conv2 + b2), is_training) 

        weights.append((w1, w2))
        biases.append((b1, b2))
        convs.append((conv1, conv2))

        # Ling's modify: 'same' replaces 'valid', no need to change size
        #size -= 4
        if layer < layers - 1:
    ...
    for layer in range(layers - 2, -1, -1):
        ...
        #h_deconv_concat = crop_and_concat(dw_h_convs[layer], h_deconv) #FIX
        ...
        h_conv_BN = batch_norm_wrapper(h_conv, is_training)
        conv2 = conv2d(h_conv_BN, w2, keep_prob)
        #conv2 = conv2d(h_conv, w2, keep_prob)#FIX
        in_node = batch_norm_wrapper(tf.nn.relu(conv2 + b2), is_training)
        #in_node = tf.nn.relu(conv2 + b2)#FIX
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=1, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        # logits: output map
        logits, self.variables, self.offset = create_conv_net(self.x, self.keep_prob, channels, n_class, self.is_Training, **kwargs)
        ...
        # Ling's modify: eliminate softmax
...
class Trainer(object):
    ...
    def _get_optimizer(self, training_iters, global_step):
        if self.optimizer == "momentum":
            ...
            decay_rate = self.opt_kwargs.pop("decay_rate", 0.95)
            # Ling's modify: adapting learning rate
...
#                    self.opt_kwargs.pop("learning_rate", 0.0001)
    ...
    def store_prediction(self, sess, batch_x, batch_y, name):
        ...
        logging.info("Verification error= {:.1f}%, loss= {:.5f}".format(error_rate(prediction,
        ...
        if len(prediction.shape) == 3:
            ...
            imsave("%s/%s_prediction.tif" % (self.prediction_path, name), temp.astype(np.float32))
        else:
            out = prediction.reshape(-1, prediction.shape[2])
            imsave("%s/%s_prediction.tif" % (self.prediction_path, name), out.astype(np.float32))
            imsave("%s/%s_combination.tif" % (self.prediction_path, name), img.astype(np.float32))
        #tif.imsave("prediction.tif", temp, imagej=True)
        return pred_shape
